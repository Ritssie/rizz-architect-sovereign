    ...
    with col_r:
        if st.session_state.state:
            ...
            if opts and len(opts) > w_idx:
                w = opts[w_idx]
                zin = w.get('zin', "Geen tekst gevonden")
                psych = w.get('psychology', "N/A")
            ...
            for i, opt in enumerate(opts):
                if i != w_idx:
                    a_zin = opt.get('zin', "Geen tekst")
                    a_psych = opt.get('psychology', "N/A")
                    a_type = opt.get('type', 'ALT')
                    st.markdown(f"""
