...
# ==============================================================================
# --- 1. CORE SYSTEM CONFIGURATION (CRASH PREVENTION) ---
# ==============================================================================
# Forceert UTF-8 encoding om de 'ascii' codec error te voorkomen
...
def get_analysis(client, b64, ctx, dark):
    ...
    prompt = f"Role: Sovereign Architect ({mode}). Output JSON: success_rate(int), green_flags(max3), red_flags(max3), options[type, zin, psychology], winner_idx."
    try:
        res = client.chat.completions.create(
            model="grok-4.20-0309-non-reasoning", # Of gebruik grok-2-vision-1212
        ...
        if isinstance(data.get('success_rate'), float) and data['success_rate'] <= 1.0:
            data['success_rate'] = int(data['success_rate'] * 100)
        return data
...
if not api_key:
    ...
    with col_l:
        ...
        # Briefing sectie
        tip_5 = "<b>The Takeaway:</b> Bereidheid om weg te lopen is macht." if is_dark else "<b>Value Offering:</b> Toon een boeiend leven."
        intel_html = f"""
            <div style='font-size:0.85rem; line-height:1.7; opacity:0.95;'>
                ...
                <div style='margin-bottom:8px;'><span style='color:#fcd34d;'>💎</span> <b>Anti-Desperation:</b> Schaarste is waarde.</div>
...
st.markdown("<br><div style='text-align:center; opacity:0.1; font-size:0.6rem; letter-spacing:2px;'>SOVEREIGN ENGINE V8.5 | ANTI-DARK OPS SHIELD</div>", unsafe_allow_html=True)
