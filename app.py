...
# --- 2. FORCED DARK MODE & MOBILE CSS (Updated for Tabs) ---
st.markdown(f"""
    <style>
    ...
    .stTabs [data-baseweb="tab"] {{
        background-color: #0d1117; border: 1px solid rgba(252, 211, 77, 0.2);
        color: #e2e8f0 !important; border_radius: 10px; padding: 10px 20px;
    }}
    .stTabs [data-baseweb="tab"]:hover {{ border: 1px solid #fcd34d; }}
    .stTabs [data-baseweb="tab"][aria-selected="true"] {{
        background-color: #fcd34d !important; color: #010409 !important; font-weight: 700;
    }}
    ...
    /* Chat Styling */
    .stChatMessage {{ background-color: #0d1117 !important; border: 1px solid rgba(252, 211, 77, 0.1); border-radius: 10px; }}
    </style>
...
texts = {
    "NL": {
        "access": "SOVEREIGN TOEGANG", "key": "OPENAI API KEY", "logistics": "Logistiek",
        "m_city": "JOUW STAD", "t_city": "HAAR STAD", "reset": "RESET SYSTEEM",
        "tab_analyze": "🔍 Screenshot Analyse", "tab_spar": "🥊 Sparring Simulator",
        "intake": "Tactical Intake", "context": "Extra Context", "scan": "⚡ EXECUTEER SCAN",
        "wait": "Analyseert patronen...", "armor": "📍 Logistiek & Armor", "weather": "Weer in",
        "outfit": "Outfit", "pick": "🏆 THE ARCHITECT'S PICK", "strategy": "STRATEGIE",
        "warning": "⚠️ Voer API Key in de sidebar in.", "info": "Wachten op tactische data...",
        "sim_start": "Start de simulatie...", "sim_end": "Simulatie beëindigd.",
        "archetype": "Kies Archetype", "chat_placeholder": "Typ je bericht...",
        "coach": "👨‍🏫 VRAAG DE ARCHITECT", "coach_wait": "Architect analyseert je status...",
        "bot_cold": "IJskoud & Afwijzend", "bot_spicy": "Pittig & Uitdagend", "bot_professional": "Zakelijk & Gereserveerd"
    },
    "EN": {
        "access": "SOVEREIGN ACCESS", "key": "OPENAI API KEY", "logistics": "Logistics",
        "m_city": "YOUR CITY", "t_city": "HER CITY", "reset": "RESET SYSTEM",
        "tab_analyze": "🔍 Screenshot Analysis", "tab_spar": "🥊 Sparring Simulator",
        "intake": "Tactical Intake", "context": "Extra Context", "scan": "⚡ EXECUTE SCAN",
        "wait": "Analyzing patterns...", "armor": "📍 Logistics & Armor", "weather": "Weather in",
        "outfit": "Armor (Outfit)", "pick": "🏆 THE ARCHITECT'S PICK", "strategy": "STRATEGY",
        "warning": "⚠️ Enter API Key in sidebar to activate.", "info": "Waiting for tactical data...",
        "sim_start": "Start the simulation...", "sim_end": "Simulation ended.",
        "archetype": "Choose Archetype", "chat_placeholder": "Type your message...",
        "coach": "👨‍🏫 ASK THE ARCHITECT", "coach_wait": "Architect is analyzing your status...",
        "bot_cold": "Ice Cold & Rejecting", "bot_spicy": "Spicy & Challenging", "bot_professional": "Business & Reserved"
    }
}
t = texts[lang]
...
if 'current_archetype' not in st.session_state: st.session_state.current_archetype = None
...
if not user_api_key:
    ...
    # --- TAB 2: SPARRING SIMULATOR ---
    with tab2:
        st.markdown(f"<div class='label-tag'>{t['tab_spar']}</div>", unsafe_allow_html=True)
        
        if not st.session_state.sim_active:
            c_arc1, c_arc2 = st.columns([1, 2])
            with c_arc1:
                archetype = st.selectbox(t['archetype'], [t['bot_spicy'], t['bot_cold'], t['bot_professional']])
            with c_arc2:
                st.write("") # Spacer
                if st.button(t['sim_start']):
                    st.session_state.sim_active = True
                    st.session_state.current_archetype = archetype
                    st.session_state.chat_history = [{"role": "assistant", "content": "Hey."}] # Simple opener
                    st.rerun()
        else:
            st.write(f"**Sparring Partner:** {st.session_state.current_archetype}")
            ...
            if prompt := st.chat_input(t['chat_placeholder']):
                ...
                with st.chat_message("assistant"):
                    with st.spinner("..."):
                        client = OpenAI(api_key=user_api_key)
                        bot_sys_msg = f"""Jij bent een vrouw op een datingapp. Taal: {lang}. Archetype: {st.session_state.current_archetype}. 
                        Reageer realistisch op de gebruiker. Houd het gesprek gaande, maar maak het niet te makkelijk."""
                        
                        # Build message list for AI
                        messages = [{"role": "system", "content": bot_sys_msg}] + st.session_state.chat_history
            ...
            if st.button(t['coach']):
                with st.spinner(t['coach_wait']):
                    client = OpenAI(api_key=user_api_key)
                    coach_sys_msg = f"""Jij bent '👨‍🏫 Rizz Architect: Coach Mode'. Analyseer de chatgeschiedenis.
                    Geef korte, directe feedback in het {lang}. Focus op statusverlies, frame control en vibe.
                    Vertel de gebruiker wat hij goed deed en wat hij beter moet doen."""
                    
                    coach_messages = [{"role": "system", "content": coach_sys_msg}] + st.session_state.chat_history
                    
                    coach_res = client.chat.completions.create(
                        model="gpt-4o-mini",
                        messages=coach_messages
                    )
                    coach_feedback = coach_res.choices[0].message.content
                    st.markdown(f"<div class='glass-card' style='border: 2px solid #fcd34d;'><div class='label-tag'>👨‍🏫 Architect Feedback</div>{coach_feedback}</div>", unsafe_allow_html=True)
