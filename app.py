...
st.markdown(f"""
    ...
    .pick-card {{ background: linear-gradient(145deg, rgba(252, 211, 77, 0.08) 0%, rgba(1, 4, 9, 1) 100%); border: 2px solid #fcd34d; border-radius: 12px; padding: 20px; margin-bottom: 25px; position: relative; }}
    .pick-badge {{ position: absolute; top: -12px; left: 20px; background: #fcd34d; color: #010409; font-family: 'JetBrains Mono', monospace; font-weight: 800; font-size: 0.7rem; padding: 2px 10px; border-radius: 4px; }}
            ...
            if st.button(t["btn_scan"]):
                with st.spinner("Analyzing..."):
                    try:
                        ...
                        sys_p = f"Role: Mastermind. Language: {l_key}. Return ONLY JSON. Format: {{'dynamics':'','success_rate':0,'green_flags':[],'red_flags':[],'options':[{{'type':'','zin':''}}],'verdict':{{'idx':0,'logic':''}}}}"
                        ...
                        if isinstance(result, dict):
                            st.session_state.rizz_master = result
                            st.rerun()
                    except Exception as e: st.error(f"Scan Error: {str(e)}")

    with c2:
        # HIER GEBEURDE DE ERROR - NU BEVEILIGD
        if st.session_state.rizz_master and isinstance(st.session_state.rizz_master, dict):
            ...
            st.progress(int(data.get('success_rate', 50)) / 100)
            ...
            if opts and len(opts) > 0:
                winner = opts[v_idx] if v_idx < len(opts) else opts[0]
                st.markdown(f"""
                <div class="pick-card">
                    <div class="pick-badge">{t['choice_header']}</div>
                    <div class="pick-type">{winner.get('type', 'Tactic')}</div>
                """, unsafe_allow_html=True)
                st.code(winner.get('zin', ''), language=None)
                st.markdown(f"""
                    <div class="logic-box">
                        <b>{t['logic_label']}:</b><br>{data.get('verdict', {}).get('logic', 'N/A')}
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                for i, o in enumerate(opts):
                    if i != v_idx:
                        st.markdown(f"<div class='dim-box'><div class='dim-type'>{o.get('type', 'Alternative')}</div>", unsafe_allow_html=True)
